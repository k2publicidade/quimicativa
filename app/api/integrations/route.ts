...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (current) await db.prepare("UPDATE erp_integrations SET provider=?,base_url=?,orders_path=?,customers_path=?,api_token=?,secret_api_token=?,active=?,updated_at=? WHERE id=?").bind(String(body.provider || "REST"), baseUrl, ordersPath, customersPath, token, secret, body.active ? 1 : 0, now, current.id).run();
    else await db.prepare("INSERT INTO erp_integrations (provider,base_url,orders_path,customers_path,api_token,secret_api_token,active,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(String(body.provider || "REST"), baseUrl, ordersPath, customersPath, token, secret, body.active ? 1 : 0, actor.userId, now, now).run();
    const row = await db.prepare("SELECT * FROM erp_integrations ORDER BY id DESC LIMIT 1").first<Integration>();
    return NextResponse.json({ integration: safe(row ?? null) });
  } catch (error) {
